...
function MenFashionDetails() {
  ...
  useEffect(() => {
    fetch(`http://localhost:5001/api/menfashion/${productId}`)
      .then((response) => {
        if (!response.ok) {
        ...
        <Row>
          ...
          <Col xs={12} md={6} lg={6}  >
            <Card style={{ border: 'none', borderBottom: '2px solid lightgray' }}>
              ...
              <Card.Text style={{ fontWeight: 'bold', color: 'black', fontSize: '30px', padding:'10px' }}>₹{product.price}</Card.Text>
              <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '10px',border:'none', borderTop:'2px solid lightgrey', padding:'10px' }}>
...
export default MenFashionDetails;
